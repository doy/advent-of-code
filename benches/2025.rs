#[macro_use]
mod helpers;

#[path = "../src/bin/2025/day1.rs"]
mod day1;
#[path = "../src/bin/2025/day10.rs"]
mod day10;
#[path = "../src/bin/2025/day11.rs"]
mod day11;
#[path = "../src/bin/2025/day2.rs"]
mod day2;
#[path = "../src/bin/2025/day3.rs"]
mod day3;
#[path = "../src/bin/2025/day4.rs"]
mod day4;
#[path = "../src/bin/2025/day5.rs"]
mod day5;
#[path = "../src/bin/2025/day6.rs"]
mod day6;
#[path = "../src/bin/2025/day7.rs"]
mod day7;
#[path = "../src/bin/2025/day8.rs"]
mod day8;
#[path = "../src/bin/2025/day9.rs"]
mod day9;
#[path = "../src/bin/2025/day12.rs"]
mod day12;
// NEXT MOD

day!(2025, 1, day1);
day!(2025, 2, day2);
day!(2025, 3, day3);
day!(2025, 4, day4);
day!(2025, 5, day5);
day!(2025, 6, day6);
day!(2025, 7, day7);
day!(2025, 8, day8);
day!(2025, 9, day9);
day!(2025, 10, day10);
day!(2025, 11, day11);
day!(2025, 12, day12);
// NEXT DAY

fn bench_2025(c: &mut criterion::Criterion) {
    c.bench_function("2025", |b| {
        b.iter(|| {
            day_combined!(2025, 1, day1);
            day_combined!(2025, 2, day2);
            day_combined!(2025, 3, day3);
            day_combined!(2025, 4, day4);
            day_combined!(2025, 5, day5);
            day_combined!(2025, 6, day6);
            day_combined!(2025, 7, day7);
            day_combined!(2025, 8, day8);
            day_combined!(2025, 9, day9);
            day_combined!(2025, 10, day10);
            day_combined!(2025, 11, day11);
            day_combined!(2025, 12, day12);
            // NEXT DAY COMBINED
        })
    });
}
criterion::criterion_group!(benches_2025, bench_2025);

criterion::criterion_main!(
    benches_2025,
    bench_2025day1,
    bench_2025day2,
    bench_2025day3,
    bench_2025day4,
    bench_2025day5,
    bench_2025day6,
    bench_2025day7,
    bench_2025day8,
    bench_2025day9,
    bench_2025day10,
    bench_2025day11,
    bench_2025day12,
    // NEXT GROUP
);
